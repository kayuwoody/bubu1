...
// ============= HEADER =============
function Header({ tweaks, onCartClick, cartCount, viewport }) {
  ...
  return (
    <header style={{
      ...
      <img src="assets/co-logo.png" alt="Coffee Oasis" style={{
        height: compact ? 36 : 44, width: "auto", objectFit: "contain"
      }}/>
      {!compact && (
        <div style={{ marginLeft: 12, fontFamily: "'Nunito', system-ui", color: tweaks.inkColor, fontSize: 13, display: "flex", alignItems: "center", gap: 6, opacity: .75 }}>
      ...
      <div style={{ marginLeft: "auto", display: "flex", gap: 10, alignItems: "center" }}>
        {!compact && (
          <button style={{
            border: `1.5px solid ${hex(tweaks.inkColor, .15)}`,
            background: "transparent", color: tweaks.inkColor,
            borderRadius: 999, padding: "10px 16px",
            fontFamily: "'Baloo 2', system-ui", fontWeight: 600, fontSize: 14,
            cursor: "pointer",
          }}>Sign in</button>
        )}
        <button onClick={onCartClick} aria-label="Cart" style={{
          ...
          cursor: "pointer",
          boxShadow: "0 6px 0 rgba(58,36,20,.18)",
        }}>
...
// ============= HERO =============
function Hero({ tweaks, viewport }) {
  const compact = viewport === "mobile";
  const showMascotBig = tweaks.mascotProminence === "hero";
  ...
  return (
    <section style={{
      ...
      padding: compact ? "20px 18px" : "28px 32px",
      display: "flex", alignItems: "center", gap: compact ? 8 : 24,
      color: "#fff",
      overflow: "hidden",
      position: "relative",
      boxShadow: `0 10px 0 ${hex(tweaks.primaryColor, .2)}, 0 18px 40px ${hex(tweaks.inkColor, .12)}`,
    }}>
      {/* bg dots */}
      ...
      <div style={{ flex: 1, position: "relative" }}>
        <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
          ...
          }}><Icon.Bolt width="12" height="12"/> Ready in ~4 min</span>
        </div>
        <h1 style={{
          margin: 0,
          fontFamily: "'Baloo 2', system-ui",
          ...
          lineHeight: 1,
          letterSpacing: "-.01em",
          textShadow: "0 2px 0 rgba(58,36,20,.25)",
        }}>Coffee, sorted.<br/>Skip the line.</h1>
        <p style={{
          margin: "10px 0 0", maxWidth: 420,
          fontFamily: "'Nunito', system-ui", fontSize: compact ? 14 : 16,
          opacity: .95, lineHeight: 1.4,
        }}>Order ahead, we'll hand it over at the counter when you pull in. Paper bag ready for the drive.</p>
      </div>
      {showMascot && (
        <div style={{
          position: "relative",
          width: compact ? (showMascotBig ? 140 : 100) : (showMascotBig ? 220 : 160),
          flexShrink: 0,
        }}>
          <img src="assets/co-mascot.png" alt="Coffee Oasis mascot"
               style={{ width: "100%", height: "auto", display: "block",
                        filter: "drop-shadow(0 8px 0 rgba(58,36,20,.2))",
                        transform: "rotate(-4deg)" }}/>
          {tweaks.showStickers && (
            <div style={{ position: "absolute", top: -6, right: -4 }}>
              <Sticker bg="#fff" color={tweaks.primaryColor} rotate={10}>Hi there!</Sticker>
            </div>
          )}
        </div>
      )}
...
function CatBar({ tweaks, cats, active, setActive, viewport }) {
  ...
  return (
    <div style={{
      position: "sticky", top: compact ? 60 : 76, zIndex: 10,
      background: `linear-gradient(to bottom, ${tweaks.bgColor} 70%, transparent)`,
      padding: compact ? "14px 16px 10px" : "18px 28px 12px",
    }}>
      <div style={{
        ...
        {cats.map(c => {
          ...
          return (
            <button key={c.id} onClick={()=>setActive(c.id)} style={{
              ...
              fontFamily: "'Baloo 2', system-ui", fontWeight: 700, fontSize: 14,
              cursor: "pointer",
              boxShadow: on ? `0 4px 0 ${hex(tweaks.inkColor, .25)}` : `0 2px 0 ${hex(tweaks.inkColor, .08)}`,
              transition: "all .12s",
            }}>{c.label}</button>
          );
        })}
...
function ItemCard({ item, qty, onAdd, onSub, onCustomize, tweaks, viewport }) {
  ...
  return (
    <div style={{
      ...
      display: "flex", gap: compact ? 12 : 14,
      alignItems: "center",
      ...
      <div style={{
        width: compact ? 72 : 84, height: compact ? 72 : 84, flexShrink: 0,
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{
          ...
          fontSize: compact ? 17 : 18, color: tweaks.inkColor,
          lineHeight: 1.1,
        }}>{item.name}</div>
        <div style={{
          fontFamily: "'Nunito', system-ui", fontSize: 13,
          color: hex(tweaks.inkColor, .6),
          marginTop: 3, lineHeight: 1.3,
        }}>{item.desc}</div>
        <div style={{
          fontFamily: "'Baloo 2', system-ui", fontWeight: 700,
          fontSize: compact ? 16 : 17, color: tweaks.inkColor,
          marginTop: 6,
        }}>RM {item.price.toFixed(2)}</div>
      </div>
      {qty > 0 && !isDrink ? (
        <div style={{
          display: "flex", alignItems: "center", gap: 10,
          ...
          <button onClick={onSub} aria-label="Decrease" style={qtyBtn("#fff", tweaks.inkColor)}><Icon.Minus width="16" height="16"/></button>
          <span style={{ fontFamily: "'Baloo 2', system-ui", fontWeight: 800, minWidth: 16, textAlign: "center" }}>{qty}</span>
          <button onClick={onAdd} aria-label="Increase" style={qtyBtn("#fff", tweaks.inkColor)}><Icon.Plus width="16" height="16"/></button>
        </div>
      ) : isDrink ? (
        <button onClick={onCustomize} aria-label={`Customize ${item.name}`} style={{
          ...
          display: "grid", placeItems: "center",
          cursor: "pointer",
          boxShadow: `0 4px 0 ${hex(tweaks.primaryColor, .4)}`,
          flexShrink: 0,
        }}>
...
function Menu({ tweaks, viewport }) {
  ...
  return (
    <div style={{ background: tweaks.bgColor, minHeight: "100%", color: tweaks.inkColor, fontFamily: "'Nunito', system-ui" }}>
      <Header tweaks={tweaks} onCartClick={()=>setCartOpen(true)} cartCount={count} viewport={viewport}/>
      <Hero tweaks={tweaks} viewport={viewport}/>
      {tweaks.showReorder !== false && window.CO_ReorderTile && React.createElement(window.CO_ReorderTile, { tweaks, viewport, onReorder: handleReorder })}
      <PickupBar tweaks={tweaks} viewport={viewport} pickup={pickup} setPickup={setPickup} etaMin={4}/>
      {tweaks.showLoyalty !== false && window.CO_LoyaltyStrip && React.createElement(window.CO_LoyaltyStrip, { tweaks, viewport })}
      <CatBar tweaks={tweaks} cats={window.CO_DATA.categories} active={activeCat} setActive={setActiveCat} viewport={viewport}/>
      <main style={{
        padding: viewport === "mobile" ? "0 16px 100px" : "0 28px 40px",
        display: "grid",
