...
export async function GET() {
  const [productsRes, settingsRes, branchRes, catsRes, defaultsRes] = await Promise.all([
    ...
    supabase
      ...
      .select('product_id, selection_group, linked_product_name')
      .eq('is_default', true),
  ]);
  ...
  const modDefaultsByProduct: Record<string, { group: string; name: string }[]> = {};
  for (const row of defaultsRes.data ?? []) {
    ...
    modDefaultsByProduct[row.product_id].push({ group: row.selection_group, name: row.linked_product_name });
  }
